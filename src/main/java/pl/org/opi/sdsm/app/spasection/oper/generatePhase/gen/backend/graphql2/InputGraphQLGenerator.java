...
public class InputGraphQLGenerator extends AbstractInputOutputGraphQLGenerator {
    ...
    private void gen(MetaNode metaNode) {
        ...
        String gqlClassName = metaNode.getFldClassSimpleName() + GRAPH_QL_INPUT_SUFFIX;
        s += GRAPH_QL_INPUT_TYPE;
        s += " ";
        s += gqlClassName + " {" + "\n";
        for(var f: metaNode.getSubs()) {
            s += "    " + f.getFldName() + ": " + getFldTypeAsStr(f.getVariant(), f.getFldClassSimpleName(), f, GRAPH_QL_INPUT_SUFFIX) + "\n";
        }
        s += "}" + "\n";
